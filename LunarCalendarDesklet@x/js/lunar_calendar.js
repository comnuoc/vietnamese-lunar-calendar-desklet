function LunarDateManager(instanceId) {
    this._init(instanceId);
}

LunarDateManager.prototype = {
    TK13: [
    ...
    LunarDate: function (dd, mm, yy, leap, jd) {
        this.day = dd;
        this.month = mm;
        this.year = yy;
        this.leap = leap;
    ...
    decodeLunarYear: function (yy, k) {
        let monthLengths, regularMonths, offsetOfTet, leapMonth, leapMonthLength, solarNY, currentJD, j, mm, ly, i;
    ...
    findLunarDate: function (jd, ly) {
        ...
        off = jd - ly[i].jd;
        ret = new this.LunarDate(ly[i].day + off, ly[i].month, ly[i].year, ly[i].leap, jd);

        return ret;
    },

    getLunarDate: function (dd, mm, yyyy) {
        let ly, jd;

        if (yyyy < 1200 || 2199 < yyyy) {
            return new this.LunarDate(0, 0, 0, 0, 0);
        }
    ...
    getSolarDate: function (dd, mm, yyyy) {
        if (yyyy < 1200 || 2199 < yyyy) {
            return new this.LunarDate(0, 0, 0, 0, 0);
        }
     ...
     */
    getSunLongitude: function (dayNumber, timeZone) {
    ...
    getDayName: function (lunarDate) {
        ...
        s = 'Ti\u1EBFt ' + this.TIETKHI[this.getSunLongitude(lunarDate.jd + 1, 7.0)] +
            ', gi\u1EDD ' + this.getCanHour0(lunarDate.jd) + ' ' + this.CHI[0] +
            ", ng\u00E0y " + cc[0] +
            ", th\341ng " + cc[1] +
            ", n\u0103m " + cc[2];
    ...
    getDayString: function () {
        return this.getSolarDateString() + ' ' + this.getCurrentTime() + " - " + this.getLunarDateString();
    },
    ...
    getCurrentTime: function () {
        let Std, Min, Sec, s1, s2, s3;

        Std = this.today.getHours();
    ...
    getGioHoangDao: function (jd) {
        let chiOfDay, gioHD, ret, count, i;
    ...
    setDate: function (date) {
        ...
        this.currentLunarDate = this.getLunarDate(
            this.today.getDate(),
    ...
    getLunarDateString: function (lunarDate) {
        let currentLunarDate = lunarDate || this.currentLunarDate;

        return "Ng\u00E0y " + currentLunarDate.day + " th\341ng " + currentLunarDate.month
            + (currentLunarDate.leap == 1 ? " nhu\u1EADn" : '')
            + " n\u0103m " + this.getYearCanChi(currentLunarDate.year);
    },

    getSolarDateString: function () {
        let s, dayOfWeek;

        dayOfWeek = this.TUAN[(this.currentLunarDate.jd + 1) % 7];
        return dayOfWeek + " " + this.today.getDate() + "/" + (this.today.getMonth() + 1) + "/" + this.today.getFullYear();
    },

    _init: function (instanceId) {
        this.FIRST_DAY = this.jdn(31, 1, 1200);
