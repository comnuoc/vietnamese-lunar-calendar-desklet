...
function Calendar(settings, lunarDateManager, eventManager, dateFormatFull) {
    this._init(settings, lunarDateManager, eventManager, dateFormatFull);
}

Calendar.prototype = {
    _init: function(settings, lunarDateManager, eventManager, dateFormatFull) {
    ...
    _update: function(forceReload) {
        ...
        let row = 2;
        let lastIterDate = new Date(iter.toUTCString());
        lastIterDate.setDate(lastIterDate.getDate() + 41);
        lastIterDate.setHours(23);
        lastIterDate.setMinutes(59);
        lastIterDate.setSeconds(59);
        ...
        while (true) {
            this.lunarDateManager.setDate(iter);
            
            let button = new St.Button({ label: iter.getDate().toString() });
            // button.reactive = false;
            let labelLunarDay = new St.Label({
                style_class: 'lunar-date' + (this.lunarDateManager.currentLunarDate.day === 1 ? ' first': '')
            });
            labelLunarDay.text = this.lunarDateManager.currentLunarDate.day
                + (this.lunarDateManager.currentLunarDate.day === 1 ? ('/' + this.lunarDateManager.currentLunarDate.month) : '');
            ...
            dayBox.add(labelLunarDay, {
                expand: false,
                y_align: St.Align.MIDDLE
            });
            // Find events.
            let dateKey = this.eventManager.getGroupByDateKey(iter);
            let dayEvents = events['events'][dateKey] ? events['events'][dateKey] : [];
            let lunarDateStr = iter.toLocaleFormat(this.dateFormatFull).capitalize() + " - " + this.lunarDateManager.getLunarDateString();
            let tooltip = new Tooltips.Tooltip(button, lunarDateStr);
            tooltip._tooltip.get_clutter_text().set_line_wrap(true);
            let markup = '\n<span weight="bold">' +
                lunarDateStr +
                '</span>\n\n' +
                '<span foreground="#C7C7C7">' +
                this.lunarDateManager.getDayName(this.lunarDateManager.currentLunarDate) +
                '</span>' +  '\n' +
                '<span foreground="#C7C7C7">' +
                this.lunarDateManager.getGioHoangDao(this.lunarDateManager.currentLunarDate.jd) +
                '</span>' +  '\n';
            // Events markup.
            if (dayEvents.length) {
                this.eventManager.sortSameDateEvents(dayEvents);
                markup += this.getEventsMarkup(dayEvents);
            }
            tooltip._tooltip.get_clutter_text().set_markup(markup);
            ...
            if (dayEvents.length) {
                styleClass += ' event-aware ' + dayEvents[0]['color'].toLowerCase();
            }

            styleClass += ' lunar-date-aware';
            dayBox.style_class = styleClass;

            let offsetCols = this.show_week_numbers ? 1 : 0;
            this.actor.add(dayBox,
    ...
    getEventsMarkup: function (events) {
        let markup = '\n\n';

        for (let i in events) {
            markup += '<span size="large" weight="bold" foreground="#75A8FF">';
            markup += events[i]['title'] ;
            markup += '</span> - ';
            markup += '<span>';
            markup += this.eventManager.getEventTimeString(events[i]);
            if (events[i]['isLunarDate']) {
                markup += ' (AL)';
            }
            markup += '</span>\n';
            if (events[i]['isLoop']) {
                markup += '<span foreground="#C7C7C7">';
                markup += 'Recurrence: ';
                markup += this.eventManager.getEventRecurrenceString(events[i]);
                markup += '</span>';
                markup += '\n';
            }
            if (events[i]['notify']) {
                markup += '<span foreground="#C7C7C7">';
                let notificationBefore = (events[i]['notifyBeforeYear'] ? events[i]['notifyBeforeYear'] + ' years ' : '')
                    + (events[i]['notifyBeforeMonth'] ? events[i]['notifyBeforeMonth'] + ' months ' : '')
                    + (events[i]['notifyBeforeDay'] ? events[i]['notifyBeforeDay'] + ' days ' : '')
                    + (events[i]['notifyBeforeHour'] ? events[i]['notifyBeforeHour'] + ' hours ' : '')
                    + (events[i]['notifyBeforeMinute'] ? events[i]['notifyBeforeMinute'] + ' minutes ' : '')
                    + (events[i]['notifyBeforeSecond'] ? events[i]['notifyBeforeSecond'] + ' seconds ' : '');
                if (notificationBefore.length) {
                    markup += 'Notification: before ' + notificationBefore;
                }
                else {
                    markup += 'Notification: true';
                }
